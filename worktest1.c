...
static tid_t curr_id; //t id for the running thread 
static tcb_t *thread_storage; //pointer to global storage 
...
kfc_init(int kthreads, int quantum_us)
{
	...
	/* !!!TODO!!! 
		might need to set the first tcb in storage to this
		call getcontext?
	*/
	thread_storage = malloc(KFC_MAX_THREADS * sizeof(tcb_t)); //allocate global thread storage
...
{
    // Grab pointer to current thread’s TCB
    tcb_t *curr_t_block = &thread_storage[curr_id];
    ...
    tcb_t *new_t_block = &thread_storage[*ptid];

    int err = 0;
    // Save the parent's context into a local variable
    ucontext_t parent_context;
    err = getcontext(&parent_context);
    if (err == -1) {
        perror("getcontext error");
        return -1;
    }

    // Now create the new thread’s context in its TCB.
    err = getcontext(&new_t_block->context);
    if (err == -1) {
        perror("getcontext error");
        return -1;
    }
    ...
    if (stack_base == NULL) {
        new_t_block->context.uc_stack.ss_sp = malloc(stack_size);
        new_t_block->context.uc_stack.ss_size = stack_size;
    } else {
        new_t_block->context.uc_stack.ss_sp = stack_base;
        new_t_block->context.uc_stack.ss_size = stack_size;
    }

    new_t_block->context.uc_stack.ss_flags = 0;
    // Set the new thread’s uc_link to the parent's saved context in the TCB.
    // (You may consider whether you want to point to the local copy or the TCB copy.)
    new_t_block->context.uc_link = &parent_context;

    // Build the new thread's context to run start_func.
    makecontext(&new_t_block->context, (void (*)()) start_func, 1, arg);
    assert(new_t_block->context.uc_stack.ss_sp != NULL);

    // Now switch from the local parent's context to the new thread's context.
    err = swapcontext(&parent_context, &new_t_block->context);
    if (err == -1) {
