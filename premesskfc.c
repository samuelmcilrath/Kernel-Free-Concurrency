		...
		caddr_t stack_base, size_t stack_size)
{
	...
	tcb_t *new_t_block =&thread_storage[*ptid]; //pointer to the tcb at index ptid 
	
	ucontext_t cont; //PROB DONT NEED ANYMORE
	...
	new_t_block->context.uc_link = &curr_t_block->context; 
	...
	//swap context
	err = swapcontext( &thread_storage[curr_id].context, &new_t_block->context);	
	if(err == -1){
		perror("swap c");
		return -1;
	}
