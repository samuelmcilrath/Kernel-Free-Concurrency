		...
		caddr_t stack_base, size_t stack_size)
{
	...
	//enqueue curr thread's id
	int temp = curr_id;
	queue_enqueue(&thread_q, &thread_storage[temp].id);

	//swap context

	curr_id = *ptid; //change curr id

	err = swapcontext(&thread_storage[temp].context, &thread_storage[*ptid].context); //shouldn't we also swap in scheduler here?
